...
async def incoming_media(client, message):
    if message.caption and message.caption.startswith(("/", "!", ".")): return None
...
async def index_receiver(client, message):
    if not is_sudo(message): return None
    ...
    if state_data and state_data.get("menu_msg_id") == message.reply_to_message.id:
        ...
        try:
            ...
            with suppress(BaseException): await message.reply_to_message.delete()
            with suppress(BaseException): await message.delete()

            if stream_msg_id:
                with suppress(BaseException): await bot_app.delete_messages(chat_id, stream_msg_id)
