...
async def worker():
    while True:
        ...
        try:
            ...
            try:
                file_path = await user_app.download_media(msg, progress=progress_bar, progress_args=(Localisation.DOWNLOAD_START, status_msg, start_time, last_up))
                ...
                while True:
                    ...
                    if "time=" in line_str:
                        ...
                        if time_match and (time.time() - last_update_time > 5):
                            ...
                            else: text = f"ℹ️ <b>ɴᴏᴡ:</b> <b>💡 ENCODING... 💡</b>\n\n⏱ `{time_match.group(1)}`"
                            try:
            ...
            if os.path.exists(custom_thumb): actual_thumb = custom_thumb
            else:
            ...
            try:
                ...
                await user_app.send_document(
                    chat_id=msg.chat.id, document=out, thumb=actual_thumb, caption=final_caption, force_document=True,
