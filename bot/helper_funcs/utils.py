...
async def kill_running_process() -> None:
...
async def delete_message_later(message, delay=10) -> None:
    await asyncio.sleep(delay)
    with suppress(BaseException): await message.delete()
...
async def send_log(msg_text: str) -> None:
...
def format_mediainfo_output(raw_info: str, file_name: str, size_str: str) -> list:
    ...
    for line in raw_info.split("\n"):
        ...
        if clean_line == "General" or clean_line.startswith(("Video", "Audio", "Text", "Menu")):
            ...
            display_name = clean_line.replace("Text", "Subtitle")
            content_json.append({"tag": "h3", "children": [f"{icon} {display_name}"]})
        elif line.strip():
            current_pre += line + "\n"
